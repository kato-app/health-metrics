...
import { changeFailureScenario, FakeGitHubSource, FakeJiraSource, jiraIssueSummary, release, releaseNote } from "./helpers/fakes.js";
...
describe("findRemediations", () => {
  ...
  it("attributes each remediation in order of trust and reports the rest as unclaimed", async () => {
    const { github, jira } = changeFailureScenario();
    ...
    assert.deepEqual(
      ...
      [
        ["revert", "v74.0", "kato-jm", "no known issue key, and the previous release v73.0 is 10.0 days older than the 3-day limit"],
        ["hotfix", "s2", "jo", "no known issue key, and the previous release s1 is 15.0 days older than the 3-day limit"],
      ],
    );
  ...
  it("does not pin a keyed hotfix on a release published after the bug was raised, and lists regression bugs without a pull request", async () => {
    ...
    const index = await buildReleaseIndex(changeFailureScenario().github, [kato], options.startDate, noopLogger);
  ...
  it("reports a patch tag whose base is not indexed as unclaimed", async () => {
    const gh = new FakeGitHubSource({ kato: [release({ id: 1, tag_name: "v69.4.1", published_at: "2026-01-10T10:00:00Z", body: releaseNote("kato", 1, "GR-1 fix") })] });
