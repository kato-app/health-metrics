...
export const changeFailure: MetricFactory = (deps): Metric => ({
  ...
  collect: async (context) => collectChangeFailure({ jira: deps.jira, github: deps.github }, await optionsFrom(deps), context),
});
...
export const unclaimedHotfixes: MetricFactory = (deps): Metric => ({
  ...
  mode: "snapshot",
  collect: async (context) => collectUnclaimedHotfixes({ jira: deps.jira, github: deps.github }, await optionsFrom(deps), context),
});
