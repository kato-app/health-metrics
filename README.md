...
**Settling and delta.** A release is judged only once `changeFailure.settlingDays` have passed since it was published, so each run writes the releases that settled since the last one. The watermark is the newest `released_at` in the tab; each run re-examines releases from the watermark minus 30 days and skips those already present, so a release that entered the index late (a prerelease promoted to a full release, say) is still caught. A skipped week needs no catching up, because the watermark does not move until rows are written. A remediating release is itself a change and gets its own row, which may in turn be flagged. Rows are ordered oldest release first.

### unclaimed-hotfixes

A **snapshot** of every hotfix, revert or patch release since `startDate` that [change-failure](#change-failure) could not pin on an earlier release, with the reason: a title with no issue key Jira knows (none at all, or one like `GR-999` that does not exist) whose previous release is too old to blame; a key raised before any indexed release of the repository, or after the fixing release itself; a fix shipped in the first indexed release of its repository; or a patch tag whose base was released before `startDate`. Columns: released_at, repo, tag, signal, evidence, author, reason. Adding or correcting the issue key in the pull request title fixes the first two kinds on the next run; the others concern causes that predate `startDate` and stay until the start date is moved past them. `--full` changes nothing here because a snapshot already covers everything.
